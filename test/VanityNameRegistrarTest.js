...
contract('VanityNameRegistrar Test', function (accounts) {
	...
	before(async () => {
		registrar = await VanityNameRegistrar.deployed();

        await registrar.setController(controllerAccount);
	});
	...
	it('should allow registration of an expired domain', async () => {
		...
		var grace = await registrar.gracePeriod();
